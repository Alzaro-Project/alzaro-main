...
const TYPE_META = {
  major: { label: 'Major',   color: 'var(--accent)', bg: 'rgba(249,115,22,.12)', border: 'rgba(249,115,22,.35)' },
...
export default function VersionBadge({ footer = false }) {
  ...
  const footerStyle = { alignSelf:'center', margin:'0 auto 8px', display:'block', background:'transparent',
      ...
      {open && (
        <div onClick={() => setOpen(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            style={{ background:'var(--surface, var(--card, #fff))', border:'1px solid var(--border, var(--line))',
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'16px' }}>
              ...
              <button onClick={() => setOpen(false)} aria-label="Close"
                style={{ background:'none', border:'none', fontSize:'20px', cursor:'pointer', color:'var(--text2, inherit)' }}>×</button>
            </div>
