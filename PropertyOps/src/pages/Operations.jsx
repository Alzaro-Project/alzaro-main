...
import { getAccountantLink, updateAccountantPermissions, revokeAccountant } from "../lib/accountant.js";
...
/* ==========================================================================
   ...
   Mirrors the SoloOps/TyreOps/GarageOps/ServiceOps AccountantTab in
   PropertyOps' own theming. Any trial/active tier can invite — no seat gate
   (this differs from the Users tab). Keys match api/accountant.js
   PRODUCTS.propertyops and the accountant RLS in migration 015.
   ========================================================================== */
// key, label, blurb — the eight visibility toggles an accountant can be granted.
const ACCT_PERMS = [
  ["dashboard",   "Dashboard",   "Portfolio totals"],
...
function AccountantTab({ user }) {
  ...
  const [perms, setPerms] = useState({ dashboard: true, properties: true, tenants: true, finance: true, maintenance: true, compliance: true, documents: true, reports: true });
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState("");
  const [notice, setNotice] = useState("");
  ...
  useEffect(() => {
    db.from("product_members").select("status").eq("user_id", user.id).eq("product", "propertyops").maybeSingle()
      ...
      <div style={cardSt}>
        ...
        {eligible && link && (
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--txt-3)", margin: "14px 0 8px" }}>What they can see — changes apply immediately</div>
            <div style={{ display: "grid", gap: 7, marginBottom: 16 }}>
              {ACCT_PERMS.map(([k, label, blurb]) => (
                <label key={k} style={{ display: "flex", gap: 9, alignItems: "flex-start", cursor: "pointer", fontSize: 12.5 }}>
                  <input type="checkbox" checked={link.permissions?.[k] === true} onChange={() => togglePerm(k)} style={{ marginTop: 2 }} />
                  <span><strong>{label}</strong><span style={{ color: "var(--txt-3)" }}> — {blurb}</span></span>
                </label>
              ))}
            </div>
