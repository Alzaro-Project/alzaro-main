// ============================================================
//  Alzaro SoloOps — Reset Password
//  Loaded by reset-password.html via <script type="text/babel">.
//  Supabase puts an access_token in the URL hash when the user
//  clicks the reset link in their email. The supabase-js client
//  picks that up automatically and creates a temporary session,
//  which lets us call updateUser({ password }).
// ============================================================
const { useState, useEffect } = React

function ResetPassword() {
  const [ready, setReady] = useState(false)      // is there a valid reset session?
  ...
  useEffect(() => {
    // supabase-js auto-parses the #access_token from the URL on load.
    // Give it a moment, then check for a session.
    const check = async () => {
      const { data } = await window.sb.auth.getSession()
      if (data.session) { setReady(true) }
      else {
        // also listen for the PASSWORD_RECOVERY event some flows fire
  ...
  const submit = async () => {
    ...
    try {
      ...
      setTimeout(() => { window.location.href = '/soloops/login' }, 1800)
    } catch (err) {
