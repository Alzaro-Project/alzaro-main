...
function AuthScreen() {
  ...
  const doRegister = async () => {
    ...
    if (!DB_READY) return setMsg("Database not connected. Add your keys in supabase.js.");
    setBusy(true);
    const { error } = await db.auth.signUp({ email, password: pw, options: { emailRedirectTo: `${window.location.origin}/confirmed?product=serviceops`, data: { company_name: company.trim(), product: "serviceops" } } });
    setBusy(false);
  ...
  useEffect(() => {
    ...
    const check = async () => {
      const { data, error } = await db.from("product_members")
        .select("id").eq("user_id", session.user.id).eq("product", "serviceops").maybeSingle();
      if (cancelled) return;
      if (error) { console.error("Membership check:", error); setMember(false); return; }
      if (data) { setMember(true); return; }
      // No membership row — if they originally registered via ServiceOps,
      // create it silently (covers pre-existing users and fresh signups).
      if (session.user.user_metadata?.product === "serviceops") {
